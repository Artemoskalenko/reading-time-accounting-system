...
def test_reading_statistics_api():
    ...
    book = Book.objects.create(**BOOK_DATA)
    book2 = Book.objects.create(**SECOND_BOOK_DATA)
    book_serialized = BookWithoutFullDescriptionSerializer(book)

    response = client.get(f"/api/v1/book-reading-statistics/{book2.id + 1}/")
    assert response.status_code == 200
    ...
    response = client.get(f"/api/v1/start-reading-session/{book2.id + 1}/")
    assert response.status_code == 200
    assert response.data["Error"] == "There is no book with this ID"
...
@pytest.mark.django_db
def test_user_reading_statistics_api():
    client = APIClient()
